...
#Beispiel 
# ========================remoteand switch =====================================
# HM-LC-Sw1PBU-FM mit Custom Firmware - 1 Kanal UP Aktor mit Stromerkennung und Wechselschaltungsfähigkeit
#   "F0A9" => {name=>"HM-LC-Sw1PBU-FM-CustomFW"        ,st=>'remoteAndSwitch'            ,cyc=>''      ,rxt=>'b'      ,lst=>'3'            ,chn=>"",},
# 1 device
# 4 kanal
...
my %listTypes = (
      ...
      regAktor       =>{							sign          =>1,    #|     literal        |          | signature (AES) options:on,off
...
$regList{1}={type => "regRemote",peers=>6};
$regList{2}={type => "regRemote",peers=>6};
$regList{3}={type => "regAktor",peers=>6};
$regList{4}={type => "regAktor",peers=>6};
